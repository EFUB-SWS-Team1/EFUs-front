...
export async function getChargeFundings(termId) {
  ...
  return unwrapList(data, ["fundings", "events"]).map(
    (item) => ({
...
/**
 * preview, 등록, 미납 청구 수정에서
 * 공통으로 사용하는 금액/대상 명세
 */
export function buildChargeAssignment({
  chargeMethod,
  targetMode,
  targetTermMemberIds,
  amount,
...
export async function getChargePaymentMembers(
  chargeId,
  params = {},
) {
  const result = await getChargePaymentMembersPage(
    chargeId,
    params,
  );
...
export async function getChargePaymentMembersPage(
  chargeId,
  params = {},
) {
  const query = {
...
export async function payChargeMember(
  chargeId,
  chargeMemberId,
  paidAt,
) {
...
export async function getChargeHistories(
  chargeId,
  params = {},
) {
  const query = {
  ...
  const payload = unwrap(data) ?? {};
  const items = unwrapList(data, ["histories"]);
  ...
  const histories = items
    .filter((item) => {
      ...
      // 납부 / 미납 / 납부 취소 제외
      if (
        /PAYMENT|PAID|UNPAID|REVERSAL|납부|미납/.test(
          historyText,
        )
      ) {
        return false;
      ...
      // 생성 / 삭제 제외
      if (
        /CREATE|CREATED|DELETE|DELETED|생성|등록|삭제/.test(
          historyText,
        )
      ) {
        return false;
      ...
      return (
        /PATCH|UPDATE|UPDATED|EDIT|EDITED|MODIFY|MODIFIED|수정|변경/.test(
          historyText,
        ) ||
        Array.isArray(
          item.changedFields,
